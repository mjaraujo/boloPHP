...
<?php endif;?>
		<li><?php echo "<?php echo \$this->Html->link(__('List').' " . Inflexao::acentos($pluralHumanName) . "', array('action' => 'index'));?>";?></li>
<?php
		$done = array();
		foreach ($associations as $type => $data) {
			foreach ($data as $alias => $details) {
				if ($details['controller'] != $this->name && !in_array($details['controller'], $done)) {
					echo "\t\t<li><?php echo \$this->Html->link(__('List').' " . Inflector::humanize(Inflexao::acentos(strtolower($details['controller']))) . "', array('controller' => '{$details['controller']}', 'action' => 'index')); ?> </li>\n";
					echo "\t\t<li><?php echo \$this->Html->link(__('New').' " . Inflector::humanize(Inflexao::acentos(Inflector::underscore($alias))) . "', array('controller' => '{$details['controller']}', 'action' => 'add')); ?> </li>\n";
